<?php
// Enhanced Expense Report Generator with Category Filtering
...
class EnhancedExpensePDF extends FPDF {
    ...
    function Header() {
        ...
        // Category Filter
        $this->SetXY(20, 42);
        $category_text = $this->filters['category_filter'] !== 'all' 
            ? 'Category: ' . $this->filters['category_filter'] 
            : 'Category: All Categories';
        $this->Cell(130, 6, $category_text, 0, 0, 'L');
    ...
    function ExecutiveSummary() {
        ...
        // Category Information (if filtered by specific category)
        if ($this->filters['category_filter'] !== 'all') {
            $this->Ln(5);
            $this->SetFont('Arial','B', 10);
            $this->SetFillColor(81, 8, 126);
            $this->SetTextColor(255, 255, 255);
            $this->Cell(260, 8, 'FILTERED BY CATEGORY: ' . strtoupper($this->filters['category_filter']), 1, 1, 'C', true);
            $this->SetTextColor(0, 0, 0);
        }
...
try {
    ...
    // Build comprehensive query with expense names and category filter
    $query = "SELECT 
        e.id,
        e.date,
        e.receipt_no,
    ...
    // Add category breakdown if there are categories (and not filtered by single category)
    if (!empty($category_totals) && $category_filter === 'all') {
    ...
    $category_suffix = '';
    if ($category_filter !== 'all') {
        $category_suffix = '_' . str_replace(' ', '_', $category_filter);
    }
    ...
    $filename = 'LATO_Financial_Report' . $type_suffix . $category_suffix . $date_suffix . '.pdf';
