...
<body id="page-top">
    ...
    <!-- Import Sidebar -->
        ...
        <?php require_once '../components/loans/officer_loan_modals.php'; ?>
       ...
       $(document).ready(function() {
           ...
           var loansTable = $('#loansTable').DataTable({
               ...
               serverSide: true,
               ajax: {
                   url: '../controllers/officer_get_paginated_loans.php', // Using officer-specific controller
                   type: 'POST'
               },
               columns: [
           ...
           // Updated modal event handlers with restrictions checking
           $(document).on('click', 'a[data-toggle="modal"][data-target^="#updateloan"]', function(e) {
                e.preventDefault();
                
                // Check if the link is disabled (for approved/disbursed loans)
           ...
           // Handle edit button in details modal with restrictions
           $('#editLoanBtn').click(function() {
               var loanId = $(this).data('id');
               
               // Check loan status before allowing edit
               checkLoanStatusBeforeEdit(loanId, function(canEdit) {
                   if (canEdit) {
       ...
       function showRestrictedActionMessage(action) {
           var message = 'You cannot ' + action + ' this loan. Only pending or denied loans can be modified.';
       ...
       // Function to check loan status before editing
       function checkLoanStatusBeforeEdit(loanId, callback) {
           $.ajax({
               ...
               success: function(response) {
                   ...
                   // Officers can only edit pending (0) or denied (4) loans
    ...
    function calculateEditLoan(loanId) {
       ...
       // Function to load loan details with officer restrictions
            function loadLoanDetails(loanId) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Show/hide Edit button based on loan status for officers
                        // Remove any existing edit disabled messages
                        $('#editDisabledMessage').remove();
                        
                        // Only allow editing for pending (0) or denied (4) loans for officers
                        if (loanStatus === 0 || loanStatus === 4) {
                            $('#editLoanBtn').show();
                        } else {
                            $('#editLoanBtn').hide();
                            
                            // Add a message about why edit is disabled for officers
                            var disabledMessage = '<div id="editDisabledMessage" class="alert alert-info mt-2">';
                            disabledMessage += '<i class="fas fa-info-circle"></i> ';
                            
                            switch(loanStatus) {
                                case 1:
                                    disabledMessage += 'This loan has been approved and cannot be edited by field officers.';
                                    break;
                                case 2:
                                    disabledMessage += 'This loan has been disbursed and cannot be edited by field officers.';
                                    break;
                                case 3:
                                    disabledMessage += 'This loan has been completed and cannot be edited by field officers.';
                                    break;
                            }
       ...
       function generateLoanModals(loanId) {
           ...
           // Fetch loan details to check status before allowing edit (Officer restrictions)
           $.ajax({
               url: '../controllers/get_loan_details.php',
               type: 'GET',
               data: { loan_id: loanId },
               ...
               success: function(response) {
                   ...
                   // Check if loan can be edited by officers (only pending=0 or denied=4)
                   if (loanStatus !== 0 && loanStatus !== 4) {
                       $('#modalLoadingOverlay').hide();
                       ...
                       switch(loanStatus) {
                           case 1:
                               statusMessage = 'This loan has been approved and cannot be edited by field officers.';
                               break;
                           case 2:
                               statusMessage = 'This loan has been disbursed and cannot be edited by field officers.';
                               break;
                           case 3:
                               statusMessage = 'This loan has been completed and cannot be edited by field officers.';
                               break;
                           default:
                               statusMessage = 'This loan cannot be edited due to its current status.';
                       }
                   ...
                   // Create the Edit Modal (only for editable loans)
                               ...
                               <div class="modal-content">
                                   ...
                                   <div class="modal-body">
                                       ...
                                       <div class="form-group">
                                           ...
                                           <small class="form-text text-muted">Officers can only set loans to Pending or Denied status.</small>
                                       </div>
                   ...
                   // Create the Delete Modal (only for editable loans)
       ...
       // Function to view loan schedule (if you have this functionality)
